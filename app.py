...
from uk_visa.constants import APP_HOST, APP_PORT
from uk_visa.pipline.prediction_pipeline import UKvisaData, UKvisaClassifier
from uk_visa.pipline.training_pipeline import TrainPipeline
...
class DataForm:
    ...
    async def get_ukvisa_data(self):
...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
            "ukvisa.html",{"request": request, "context": "Rendering"})
...
async def predictRouteClient(request: Request):
    try:
        ...
        await form.get_ukvisa_data()
        
        ukvisa_data = UKvisaData(
        ...
        ukvisa_df = ukvisa_data.get_ukvisa_input_data_frame()

        model_predictor = UKvisaClassifier()

        value = model_predictor.predict(dataframe=ukvisa_df)[0]
